...
const bsearch = (arr, x, start, end) => { 
    ...
    const sum = arr[mid] + x;  
    if ( sum === 2020 ) return arr[mid] * x; 
    return sum > 2020 ? bsearch(arr, x, start, mid-1) : bsearch(arr, x, mid+1, end);

} 
  
console.log(a.map(x => bsearch(a, x, 0, a.length - 1)).filter(x => x)[0])

const passports = `byr:2024 iyr:2016
...
console.log(passports.map(passport => passport.split(/\s+/).reduce((acc, pline) => ({...acc, [pline.split(':')[0]]: pline.split(':')[1]}), {})).filter(passObj => {
    if (!passObj.byr || parseInt(passObj.byr) < 1920 || parseInt(passObj.byr) > 2002 ) return false;
    if (!passObj.iyr || parseInt(passObj.iyr) < 2010 || parseInt(passObj.iyr) > 2020 ) return false;
    if (!passObj.eyr || parseInt(passObj.eyr) < 2020 || parseInt(passObj.eyr) > 2030 ) return false;
    if (!passObj.hgt || !/^\d+(in|cm)$/.test(passObj.hgt)) return false;
    if (passObj.hgt.includes('cm')) {
        if (parseInt(passObj.hgt.split('cm')[0]) < 150 || parseInt(passObj.hgt.split('cm')[0]) > 193 ) return false;
    }
    if (passObj.hgt.includes('in')) {
        if (parseInt(passObj.hgt.split('in')[0]) < 59 || parseInt(passObj.hgt.split('in')[0]) > 76) return false;
    }
    if (!passObj.hcl || !/^#[0-9a-f]{6}$/.test(passObj.hcl)) return false;
    if (!['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(passObj.ecl) || passObj.ecl.length != 3) return false;
    if (!passObj.pid || !/^\d{9}$/.test(passObj.pid)) return false;
    return true;
}).length)
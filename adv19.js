...
const [_rules, _messages] = input.split('\n\n')

const messages = _messages.split('\n')
const rules = _rules.split('\n').sort((a,b) => parseInt(a.split(':')) - parseInt(b.split(':')))
    .map(line => line.split(':'))
    .map(
        rule => `rule${rule[0]} -> ` + rule[1].split('|')
            .map(_rule => _rule.match(/([a-z0-9]+)/g)
            .map(r => parseInt(r) ? 'rule' + r: `"${r}"`).join(' ')
        )
    )
    .map(el => el.replace(',','\n\t| ')).join('\n')
//    .map((el, idx) => `rule${idx} -> ${el.join('\n\t| ')}`).join('\n')
...
grammar.ParserStart = 'rule0';


console.log(messages.filter(msg => {
    try {
        const parser = new nearley.Parser(nearley.Grammar.fromCompiled(grammar));
        parser.feed(msg)
        // console.log(msg, parser.results.length ,true)

...
console.log(
    groupAnswers.map(answers => {
        const sum = {}
        answers.forEach(person => person.forEach(personAnswers => { sum[personAnswers] = !sum[personAnswers] ? 1 : sum[personAnswers] + 1 }))
        
        return (Object.keys(sum)).filter(ans =>  sum[ans] === answers.length).length

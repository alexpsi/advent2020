const rules = `light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
clear brown bags contain no other bags.`.split('\n').map(line => line.split('contain')).reduce((acc, rule) => {
    const bagKey = rule[0].match(/(\S*\s\S*)\sbags/)[1];
    const parsedRule = rule[1].match(/\d+\s\S*\s\S*/g);
    if (parsedRule) {
        acc[bagKey] = parsedRule.reduce(
            (_acc, value) => { _acc[value.slice(2)] = parseInt(value[0]); return _acc; }, {}
        )
    }
    return acc;
}, {})
...
const canContainGoldBag = (bagKey) => {
    if (!rules[bagKey]) return 0;
    return Object.keys(rules[bagKey]).reduce((acc, _bagKey) => _bagKey === 'shiny gold' ? acc + rules[bagKey]['shiny gold'] : acc + canContainGoldBag(_bagKey), 0); 
}

console.log(Object.keys(rules).filter(bagColor => canContainGoldBag(bagColor) > 0).length)

const containedBags = (bagKey) => {
    if (!rules[bagKey]) return 0;
    return Object.keys(rules[bagKey]).reduce((acc, _bagKey) => acc + (rules[bagKey][_bagKey] + rules[bagKey][_bagKey] * containedBags(_bagKey)), 0);
}
